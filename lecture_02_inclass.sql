...
/* 1.1 Hàm Scalar: OnlyoneVlue should be a number show datetime 
...
-- PROCEDURE doesnt have the RETURNS
BEGIN 
...
*/
ALTER FUNCTION func_DayTillChristmas (
    @date DATE
) RETURNS INT AS 
BEGIN 
    RETURN DATEDIFF(DAY, @date,CONCAT(YEAR(@date),'-12-24'))
END 
GO 

SELECT dbo.func_DayTillChristmas('2021-10-01')
GO
/* Sửa đổi hàm
...
BEGIN 
    RETURN DATEDIFF(WEEK, @date,DATEFROMPARTS(YEAR(@date)+1,12,24))
END 
GO 
...
select 
    OrderID,
    ShippedDate,
    dbo.func_DayTillChristmas(ShippedDate) DayTillChristmas 
from dbo.Orders
...
/* 1.2 Hàm Table :Scalar: INT and Vachar
...
*/
ALTER FUNCTION func_OrdersShippedToCountry(
    @country NVARCHAR(50)
) RETURNS TABLE AS 
RETURN  
    SELECT *
...
SELECT * 
FROM dbo.func_OrdersShippedToCountry('USA') A
LEFT JOIN dbo.[Order Details] B on A.OrderID=B.OrderID
GO
...
CREATE FUNCTION func_Top10PercentProduct(
    @orderYear INT
) RETURNS TABLE AS 
RETURN  
...
SELECT * 
FROM dbo.func_Top10PercentProduct(1996)
GO
...
/* Lưu ý:
    ...
    SELECT, UPDATE, DELETE, INSERT ...--> SQL STATMENT(LÀ SQL)
    - Block lệnh là những đoạn code scripting tính toán và phải bắt đầu bằng BEGIN...END.Viết trên sever SQL bản chất là ngôn ngữ lập trình T-SQL  Là Ngôn ngữ lập trình SQL
...
-- block lệnh
BEGIN 
    DECLARE @year INT --Khai báo biến <tên biến. định dạng dữ liệu 
    SET @year = 1997  --Select sựa trên giá trị biến
    SELECT * 
    FROM dbo.Orders 
...
ALTER FUNCTION func_OddEven(@number INT)
...
SELECT EmployeeID,dbo.func_OddEven(EmployeeID) 
FROM dbo.Orders 
GO 
